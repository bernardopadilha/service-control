...
const statusOrder = ['Chris Turra', 'Fabiano de Lima', 'Emerson Silvera']

export function OrderCardTechnical({
  title,
  description,
  type,
}: {
  ...
  async function incrementTechnicsSkills({
    technical,
    skillType,
    oldValue,
  }: any) {
  ...
  async function decrementTechnicsSkills({
    technical,
    skillType,
    oldValue,
  }: any) {
      ...
      <CardContent className="space-y-4">
        {technicsSkill
          ...
          .map((item: any, i: any) => (
            <div
              key={i}
              className="flex items-center justify-between border-b border-sky-500 pb-2"
            >
              <h1>{item.technical_name}</h1>

              <div className="flex items-center justify-center gap-2">
                <Button
                  onClick={() =>
                    decrementTechnicsSkills({
        ...
        <Button
          onClick={() => resetTechnicsSkills({ skillType: type })}
          type="button"
          className="w-full"
        >
          Resetar
